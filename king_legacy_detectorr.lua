-- ═══════════════════════════════════════════════════════════════════
--  King Legacy Boss Detector v7.0
--  Area: Sea 2 | Executor: Delta Mobile (Android)
--
--  v7.0: Autofight loop TP (anti jatuh air) + Store brute-force 30+ patterns
...
-- ─── Auto Store Fruit ─────────────────────────────────────────────
--
--  v6.9: Direct remote call — bypass UI sepenuhnya
--  Remote candidates: DFPassive, EtcFunction (dari daftar remote spy)
--
--  Pendekatan:
--    1. Cari nama buah yang di-equip (Tool di character)
--    2. Coba panggil DFPassive:InvokeServer dengan berbagai argumen
--    3. Cek apakah buah hilang dari character (berhasil disimpan)
--    4. Fallback: UI click jika remote gagal
--
function autoStoreFruit()
    ...
    if DFPassive then
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Collect"},                label = "DFPassive(Collect)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Collect", fruitName},     label = "DFPassive(Collect,fruit)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Store"},                  label = "DFPassive(Store)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Store", fruitName},       label = "DFPassive(Store,fruit)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Add"},                    label = "DFPassive(Add)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Add", fruitName},         label = "DFPassive(Add,fruit)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {"Put"},                    label = "DFPassive(Put)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {fruitName},                label = "DFPassive(fruit)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {fruitName, "Store"},       label = "DFPassive(fruit,Store)"})
        table.insert(storeAttempts, {remote = DFPassive, type = "invoke", args = {fruitName, "Collect"},     label = "DFPassive(fruit,Collect)"})
    end

    if EtcFunction then
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"CollectFruit"},                label = "Etc(CollectFruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"CollectFruit", fruitName},     label = "Etc(CollectFruit,fruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"StoreFruit"},                  label = "Etc(StoreFruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"StoreFruit", fruitName},       label = "Etc(StoreFruit,fruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"PassiveFruit"},                label = "Etc(PassiveFruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"PassiveFruit", fruitName},     label = "Etc(PassiveFruit,fruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"Passive", "Store"},            label = "Etc(Passive,Store)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"Passive", "Collect"},          label = "Etc(Passive,Collect)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"FruitBag", fruitName},         label = "Etc(FruitBag,fruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"Store", fruitName},            label = "Etc(Store,fruit)"})
        table.insert(storeAttempts, {remote = EtcFunction, type = "invoke", args = {"Collect", fruitName},          label = "Etc(Collect,fruit)"})
    end

    if InventoryEq then
        table.insert(storeAttempts, {remote = InventoryEq, type = "invoke", args = {"Store", fruitName},     label = "InvEq(Store,fruit)"})
        table.insert(storeAttempts, {remote = InventoryEq, type = "invoke", args = {"Collect", fruitName},   label = "InvEq(Collect,fruit)"})
        table.insert(storeAttempts, {remote = InventoryEq, type = "invoke", args = {"Passive", fruitName},   label = "InvEq(Passive,fruit)"})
    end

    if CollectFruit then
        table.insert(storeAttempts, {remote = CollectFruit, type = "fire", args = {},                        label = "CollectFruit()"})
        table.insert(storeAttempts, {remote = CollectFruit, type = "fire", args = {fruitName},               label = "CollectFruit(fruit)"})
        table.insert(storeAttempts, {remote = CollectFruit, type = "fire", args = {"Store"},                 label = "CollectFruit(Store)"})
        table.insert(storeAttempts, {remote = CollectFruit, type = "fire", args = {"Store", fruitName},      label = "CollectFruit(Store,fruit)"})
    end

    if ButtonClicked then
        table.insert(storeAttempts, {remote = ButtonClicked, type = "fire", args = {"Store"},                label = "BtnClick(Store)"})
        table.insert(storeAttempts, {remote = ButtonClicked, type = "fire", args = {"PassiveBag", "Store"},  label = "BtnClick(PB,Store)"})
        table.insert(storeAttempts, {remote = ButtonClicked, type = "fire", args = {"Collect"},              label = "BtnClick(Collect)"})
    end
    ...
    for i, attempt in ipairs(storeAttempts) do
        ...
        local still, _ = hasFruitTool()
        if not still then
            still, _ = hasBackpackFruit()
        end
    ...
    -- Fallback: UI click approach
    setStatusGUI("🍎 Remote gagal, coba UI click...", Color3.fromRGB(255,160,60), 2)
    ...
    for _, obj in ipairs(pg:GetDescendants()) do
        if (obj:IsA("TextButton") or obj:IsA("ImageButton"))
           and obj.Name == "Store"
           and obj:GetFullName():find("PassiveBagFrame") then
            storeBtn = obj
            break
        end
    end
    if not storeBtn then
        for _, obj in ipairs(pg:GetDescendants()) do
            if (obj:IsA("TextButton") or obj:IsA("ImageButton"))
               and obj.Name == "Store"
               and obj:GetFullName():find("SlotInfoFrame") then
                storeBtn = obj
                break
            end
        end
    ...
    if storeBtn then
        ...
        pcall(function()
            local p = storeBtn
            while p and p ~= pg do
                if p:IsA("GuiObject") then table.insert(uichain, 1, p) end
                p = p.Parent
            end
        end)

        pcall(function()
            for _, gui in ipairs(uichain) do
                table.insert(savedStates, {obj = gui, visible = gui.Visible, position = gui.Position})
                gui.Visible = true
                if gui:IsA("Frame") or gui:IsA("ScrollingFrame") then
                    gui.Position = UDim2.fromOffset(
                        math.max(50, gui.AbsolutePosition.X),
                        math.max(20, gui.AbsolutePosition.Y)
                    )
                end
            end
        ...
        pcall(function() firesignal(storeBtn.MouseButton1Click) end)
        task.wait(0.2)

        local absX = storeBtn.AbsolutePosition.X
        if absX > 0 then
            pcall(function()
                local pos  = storeBtn.AbsolutePosition
                local size = storeBtn.AbsoluteSize
        ...
        pcall(function()
            for i = #savedStates, 1, -1 do
                local s = savedStates[i]
                s.obj.Position = s.position
                s.obj.Visible  = s.visible
            end
        end)

        local still2, _ = hasFruitTool()
        if not still2 then
            still2, _ = hasBackpackFruit()
        end
        if not still2 then
            setStatusGUI("✅ Store berhasil! (UI click)", Color3.fromRGB(80,255,150), 5)
            return true
        end
    end

    setStatusGUI("❌ Store gagal, semua metode dicoba", Color3.fromRGB(255,80,80), 5)
    return false
end
    ...
    task.spawn(function()
        ...
        while isFighting and isRunning do
            ...
            if hrp and bossHRP and bossHRP.Parent then
                ...
                local dist = (playerPos - bossPos).Magnitude
                local falling = playerPos.Y < bossPos.Y - 3

                if dist > 5 or falling then
                    hrp.CFrame = CFrame.new(bossPos + Vector3.new(3, 2, 0))
                    hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                end
            elseif hrp and savedBossPos then
                local playerPos = hrp.Position
                if playerPos.Y < savedBossPos.Y - 3 then
