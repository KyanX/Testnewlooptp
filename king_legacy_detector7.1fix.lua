-- ═══════════════════════════════════════════════════════════════════
--  King Legacy Boss Detector v7.2
--  Area: Sea 2 | Executor: Delta Mobile (Android)
--
--  v7.2: Platform 2x2, chest tier fix (Buttom/SpawnPit/ClockTime), 4 fallback store
...
-- ═══════════════════════════════════════════════════════════════════
--  CHEST DATA v7.2 (dikonfirmasi dari scan lapangan + user data)
--
--  T1: ChestSpawner, Top, Buttom, SkullRetopo, EyeRight, EyeLeft
--  T2: ChestSpawner, ChestTop, ChestBottum, EyeRight, EyeLeft, SkullRetopo
--  T3: ChestTop, ChestBottum, Wing, Dragon
--      + jika ada Spawn Pit / EyeRight / EyeLeft di dekat → tetap T3
--  T4: Hanya ChestBottum + ChestTop (tentatif)
--
--  Island anchors: ClockTime (timer despawn) + HydraStand (platform)
--  Signal: MainMotor6D = chest sedang dibuka (rewards keluar)
-- ═══════════════════════════════════════════════════════════════════

local CHEST_SET = {
    ChestSpawner = true,
    Top = true, Buttom = true,
...
local TP_BLACKLIST = { Coins = true, MiniCoin = true, ["Spawn Pit"] = true }

local ISLAND_ANCHORS = { "ClockTime", "HydraStand" }
...
local function detectTier(nameSet)
    local hasDragon   = nameSet["Dragon"] or nameSet["Wing"]
    local hasSpawnPit = nameSet["Spawn Pit"]
    local hasTop      = nameSet["Top"] or nameSet["Buttom"]
    local hasChestTop = nameSet["ChestTop"] or nameSet["ChestBottum"]
    local hasSkull    = nameSet["SkullRetopo"]
    local hasSpawner  = nameSet["ChestSpawner"]

    if hasDragon then return "T3 🐉" end
    if hasChestTop and hasSpawnPit and not hasSpawner then return "T3 🐉" end
    if hasTop and hasSpawner then return "T1" end
    if hasTop then return "T1" end
    if hasChestTop and hasSkull and hasSpawner then return "T2" end
    if hasChestTop and hasSkull then return "T2" end
    if hasChestTop then return "T4 (?)" end
    return "?"
end
...
function autoStoreFruit()
    ...
    -- Fallback 1: getconnections() — fire Store button connections langsung
    setStatusGUI("🍎 FB1: getconnections()...", Color3.fromRGB(255,160,60), 2)
    ...
    if pg then
        ...
        for _, obj in ipairs(pg:GetDescendants()) do
            if (obj:IsA("TextButton") or obj:IsA("ImageButton")) and obj.Name == "Store" then
                table.insert(storeButtons, obj)
            end
        end

        if getconnections then
            for _, btn in ipairs(storeButtons) do
                pcall(function()
                ...
                task.wait(0.5)
                if not fruitStillExists() then
                    setStatusGUI("✅ Store! (getconn " .. btn:GetFullName():match("[^.]+$") .. ")",
                                 Color3.fromRGB(80,255,150), 5)
                    return true
                end
            end
        ...
        -- Fallback 2: firesignal() — fire MouseButton1Click signal
        setStatusGUI("🍎 FB2: firesignal()...", Color3.fromRGB(255,160,60), 2)
        if firesignal then
            for _, btn in ipairs(storeButtons) do
                pcall(function() firesignal(btn.MouseButton1Click) end)
                task.wait(0.5)
                if not fruitStillExists() then
                    setStatusGUI("✅ Store! (firesignal)", Color3.fromRGB(80,255,150), 5)
                    return true
                end
            end
        end

        -- Fallback 3: fireclick() — Delta specific click
        setStatusGUI("🍎 FB3: fireclick()...", Color3.fromRGB(255,160,60), 2)
        if fireclick then
            for _, btn in ipairs(storeButtons) do
                pcall(function() fireclick(btn) end)
                task.wait(0.5)
                if not fruitStillExists() then
                    setStatusGUI("✅ Store! (fireclick)", Color3.fromRGB(80,255,150), 5)
                    return true
                end
            end
        end
    ...
    -- Fallback 4: Scan ALL RemoteFunctions/Events di game, coba fire setiap satu
    setStatusGUI("🍎 FB4: scan all remotes...", Color3.fromRGB(255,160,60), 2)
    local allRemotes = {}
    pcall(function()
        for _, obj in ipairs(game.ReplicatedStorage:GetDescendants()) do
            if obj:IsA("RemoteFunction") or obj:IsA("RemoteEvent") then
                local nm = obj.Name:lower()
                if nm:find("passive") or nm:find("fruit") or nm:find("bag")
                   or nm:find("collect") or nm:find("inventory") or nm:find("slot") then
    ...
    for i, rem in ipairs(allRemotes) do
        setStatusGUI("🍎 FB4 " .. i .. "/" .. #allRemotes .. ": " .. rem.Name,
                     Color3.fromRGB(255,180,60), 1)
        local keywords = {"Store", "Collect", "Add", fruitName, "PassiveBag"}
        for _, kw in ipairs(keywords) do
            ...
            task.wait(0.3)
            if not fruitStillExists() then
                setStatusGUI("✅ Store! (" .. rem.Name .. "," .. kw .. ")",
                             Color3.fromRGB(80,255,150), 5)
                return true
            end
        end
    ...
    setStatusGUI("❌ Store gagal semua metode", Color3.fromRGB(255,80,80), 5)
    return false
end
